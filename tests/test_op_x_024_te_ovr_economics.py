...
spec=importlib.util.spec_from_file_location('opx24',P); m=importlib.util.module_from_spec(spec); spec.loader.exec_module(m)

def card(**kw):
    base={'position':'TE','extraction_status':'COMPLETE','program':'Core','metadata':{},'displayed_ratings':{'SPD':80},'overall':83,'archetype':'Physical Route Runner'}
...
def test_stats_reports_dispersion():
    s=m.stats([70,80,90]); assert s['n']==3 and s['range']==20 and s['median']==80
...
def test_holdout_feature_model_helpers_are_deterministic():
    ...
    b=m.ridge_fit(rows,['SPD','ACC']); assert b is not None
...
def test_component_card_never_invents_missing_values():
    ...
    d=m.component_card(c); assert d['ATHLETICISM']==81 and d['RECEIVING']==85 and d['PASS_BLOCKING'] is None

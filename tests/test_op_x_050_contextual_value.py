...
def test_context_never_mutates_frozen_evaluation():
    ...
    assert result.frozen_identity() == (84.25, 7, 92.1)
...
def test_treydez_green_identity_survives_gadget_hb_deployment():
    ...
    assert te.frozen_identity() == hb.frozen_identity()
...
def test_rgiii_and_benkert_release_context_only():
    ...
    assert rgiii.frozen_identity() == benkert.frozen_identity() == BASE.pancake_score, BASE.position_rank, BASE.position_percentile
...
def test_sherman_assignment_counterfactual():
    ...
    assert zone.frozen_identity() == man.frozen_identity()
...
def test_theoretical_build_cannot_masquerade_as_observed():
    build = RealizedBuild("candidate")
    assert build.status == "THEORETICAL"
...
def test_residual_does_not_claim_model_error():
    ...
    assert row["category"] == "HIGH_PANCAKE_REJECTION"
    assert row["model_error_claimed"] is False

...
def test_spatial_slot_association_depth_name_and_ovr():
    ...
    obs=[o('OFFENSE',.01,.01),o('WR',.01,.3),o('85',.02,.4),o('Alpha Receiver',.02,.5),
         o('WR',.21,.3),o('87',.22,.4),o('Beta Receiver',.22,.5),o('WR',.41,.3),o('Gamma Receiver',.42,.5)]
...
def test_cb_ss_and_specialist_labels_remain_distinct():
    dr=regions('CB1','CB2','CB3','SS1','SS2')
    obs=[]
    for i,name in enumerate(['A One','B Two','C Three','D Four','E Five']): obs.append(o(name,i*.2+.01,.4))
    _,found,_=extract_structured('d.png',obs,{'DEFENSE':dr},view='DEFENSE')
    assert [x.slot for x in found]==['CB1','CB2','CB3','SS1','SS2']
    sr=regions('SUBLB1','SUBLB2','RDT1','RDT2')
...
def test_matching_exact_ambiguous_unmatched_and_observed_ovr_separate():
    cards=[{'card_id':'a','player_name':'Jaylen Lewis','position':'CB','native_overall':83},
           {'card_id':'b','player_name':'Version Guy','position':'WR','native_overall':85},
           {'card_id':'c','player_name':'Version Guy','position':'WR','native_overall':87}]
    jay=Candidate('1','DEFENSE','CB1','Jaylen Lewis',85,'CB',observed_ratings={'SPD':89,'ACC':88,'AGI':89,'COD':82,'MCV':91,'ZCV':91,'PRC':84})
    assert match_candidate(jay,cards).canonical_card_id=='a'
    assert cards[0]['native_overall']==83 and jay.displayed_ovr==85 and jay.observed_ratings['MCV']==91
...
def test_specialist_assignment_dedupes_owned_card():
    a=Candidate('a','DEFENSE','SS1','Player A',canonical_card_id='card-a',match_status='MATCHED')
    b=Candidate('b','SPECIALISTS','SUBLB1','Player A',canonical_card_id='card-a',match_status='MATCHED')
    assert ownership_key(a)==ownership_key(b)=='card-a'

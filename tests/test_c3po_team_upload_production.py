    ...
    def translate_offense_tackles(self, screenshot):
        ...
        slots = {
            ...
            "RT1": TackleSlotObservation(
                ...
                (PlayerObservation("Juan Gaston", 81),),
            ),
        }
        return TackleScreenObservation(
            "OFFENSE", slots, "configured-fixture", "deterministic-hosted"
        )
...
def _multipart():
    ...
    header = (
        ...
        'filename="offense.png"\r\nContent-Type: image/png\r\n\r\n'
    ).encode()
    body = header + b"pixels\r\n" + f"--{boundary}--\r\n".encode()
...
def test_actual_team_upload_invokes_configured_c3po_and_persists(
    ...
    def accepted_ocr(state_store, gm):
        ...
        shot = state.screenshots[-1]
        shot["view"] = "OFFENSE"
        shot["extraction_status"] = "OCR READ — OFFENSE"
        state.candidates = [
            Candidate("lt", "OFFENSE", "LT1", player_name="legacy-lt"),
            Candidate("rt", "OFFENSE", "RT1", player_name="legacy-rt"),
    ...
    handler = team_app.create_handler(Path.cwd())
    ...
    restarted = TeamImportStore(
        tmp_path / "team-import.json", tmp_path / "uploads"
    ).load()

...
def test_roster_roles_reads_materialized_artifact(tmp_path, monkeypatch, capsys):
    ...
    (out / "ROSTER_ROLE_MAP.json").write_text(json.dumps({"summary": {"entries": 24}}))
...
def test_zero_coin_upgrades_preserves_unknown(tmp_path, monkeypatch, capsys):
    ...
    (out / "ZERO_COIN_UPGRADES.json").write_text(json.dumps(expected))
    assert run_cli(monkeypatch, capsys, "--root", tmp_path, "zero-coin-upgrades") == expected


def test_target_challenge_selects_exact_materialized_target(tmp_path, monkeypatch, capsys):
    out = tmp_path / "data/research/op_x_051"
    out.mkdir(parents=True)
    targets = [{"status": "AMBIGUOUS CARD VERSION", "n": n} for n in range(1, 6)]
    (out / "TARGET_CHALLENGES.json").write_text(json.dumps({"targets": targets}))
    payload = run_cli(monkeypatch, capsys, "--root", tmp_path, "target-challenge", "--index", "3")
...
def test_legacy_commands_delegate_to_existing_gm_cli(monkeypatch):
    called = []
    monkeypatch.setattr(sys, "argv", ["operation-pancake-gm", "player", "--name", "Example"])
...
def test_role_commands_dispatch_to_role_intelligence(tmp_path, monkeypatch, capsys):
    monkeypatch.setattr(cli, "role_board", lambda root, pos, role, limit: {"kind": "board", "position": pos, "role": role, "limit": limit})
    payload = run_cli(monkeypatch, capsys, "--root", tmp_path, "role-board", "cb", "man", "--limit", "7")
    assert payload == {"kind": "board", "position": "CB", "role": "MAN", "limit": 7}

    monkeypatch.setattr(cli, "role_alternatives", lambda root, cid, role, limit: {"kind": "alternatives", "card_id": cid, "role": role, "limit": limit})
    payload = run_cli(monkeypatch, capsys, "--root", tmp_path, "role-alternatives", "card-1", "zone", "--limit", "4")
    assert payload == {"kind": "alternatives", "card_id": "card-1", "role": "ZONE", "limit": 4}

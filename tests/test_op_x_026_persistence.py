...
BRANCH = "agent/op-x-012-reconcile-artifacts"
...
def test_remote_unchanged_persists(tmp_path: Path) -> None:
    ...
    assert git(work, "rev-parse", "HEAD") == remote_head(bare)
...
def test_remote_advanced_is_preserved_and_result_reapplied(tmp_path: Path) -> None:
    ...
    subprocess.check_call(
        ["git", "merge-base", "--is-ancestor", concurrent_sha, remote_head(bare)],
        cwd=bare,
    )
...
def test_conflict_fails_without_force_push(tmp_path: Path) -> None:
    ...
    assert remote_head(bare) == remote_before
...
def test_repeated_advancement_is_bounded(tmp_path: Path) -> None:
    ...
    hook.write_text(
        ...
        "git -c user.name=test -c user.email=test@example.com commit -m race-$n >/dev/null; "
        "git push >/dev/null; fi\n"
    )
    hook.chmod(0o755)
    result = run_script(
        work,
        OPX026_MAX_RETRIES="3",
        OPX026_TEST_BEFORE_PUSH_HOOK=str(hook),
    )
    assert result.returncode == 0, result.stderr
    assert "OPX026_PERSISTENCE_ATTEMPT=3" in result.stdout
    assert remote_head(bare) == git(work, "rev-parse", "HEAD")

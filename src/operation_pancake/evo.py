"""Conservative EVO decision workflow: unknown EVO science remains unknown."""
...
class EVODefinition:
    ...
    @property
    def final_attributes_known(self): return bool(self.known_attribute_boosts)
    def eligible(self, player: Mapping[str, Any]):
        basis=[]; pos=str(player.get("position") or "").upper(); arch=str(player.get("archetype") or "").casefold()
    ...
    def project(self, player: Mapping[str, Any]):
        if not self.known_attribute_boosts: return {"target_ovr":self.target_ovr,"final_attributes":None,"final_pancake_score":None,"limitations":["FINAL ATTRIBUTES UNKNOWN","FINAL PANCAKE SCORE UNKNOWN"]}
        result=dict(player); deltas={}
        ...
        for attr,boost in self.known_attribute_boosts.items():
            ...
            if isinstance(current,(int,float)):
                final=min(99,int(current)+int(boost)); result[attr]=final; deltas[attr]={"current":int(current),"boost":int(boost),"projected":final}
...
class EVOStore:
    ...
    def save(self,evos:Iterable[EVODefinition]):
        self.path.parent.mkdir(parents=True,exist_ok=True); self.path.write_text(json.dumps({"version":self.VERSION,"evos":[asdict(e) for e in evos]},indent=2,sort_keys=True)+"\n",encoding="utf-8")
...
def decide_evo(*,slot_protected,slot_rerollable,projected_improvement,replacement_improvement,replacement_cost,evo_base_cost,final_attributes_known):
    ...
    if projected_improvement is False: return {"decision":"SAVE EVO","reasons":["projected EVO state does not improve the roster slot"],"confidence":"HIGH"}
    if projected_improvement is True:
        if replacement_improvement and replacement_cost is not None and evo_base_cost is not None and replacement_cost<evo_base_cost: return {"decision":"BUY REPLACEMENT","reasons":["known improving replacement costs less than the EVO base path"],"confidence":"MEDIUM"}

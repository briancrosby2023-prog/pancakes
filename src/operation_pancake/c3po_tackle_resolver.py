...
def _identity_score(observed,canonical):
 a,b=normalize_name(observed),normalize_name(canonical);return SequenceMatcher(None,a,b).ratio() if a and b else 0.0
...
def resolve_player(observation,position,cards,slot,depth):
 ...
 if not observation.observed_name:return _unresolved(base)
 pool=[c for c in cards if _is_cfb27(c)];identities={}
 for card in pool:
  name=card.get('player_name') or ''
  if name:identities.setdefault(name,[]).append(card)
 q=normalize_name(observation.observed_name);exact=next(((n,v) for n,v in identities.items() if normalize_name(n)==q),None)
 if exact is not None:identity,variants=exact
 else:
  ranked=sorted(((_identity_score(observation.observed_name,n),n,v) for n,v in identities.items()),reverse=True);ambiguous=len(ranked)>1 and ranked[0][0]-ranked[1][0]<.08
  if not ranked or ranked[0][0]<.78 or ambiguous:return _unresolved(base)
  _,identity,variants=ranked[0]
  if not _identity_gate(observation.observed_name,identity):return _unresolved(base)
...
def resolve_tackles(observation,cards):
 ...
 for slot,position in (('LT1','LT'),('RT1','RT')):
  translated=observation.slots[slot];out.append(resolve_player(translated.starter,position,cards,slot,0));out.extend(resolve_player(row,position,cards,slot,depth) for depth,row in enumerate(translated.backups,1))
 return out
class TackleResolutionStore:
 def __init__(self,path:Path):self.path=path
 def save(self,observation,resolutions):
  self.path.parent.mkdir(parents=True,exist_ok=True);payload={'translator_observation':observation.to_dict(),'resolutions':[asdict(row) for row in resolutions]};tmp=self.path.with_suffix(self.path.suffix+'.tmp');tmp.write_text(json.dumps(payload,indent=2)+'\n',encoding='utf-8');tmp.replace(self.path)

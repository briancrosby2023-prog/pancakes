...
def resolve_player(observation: PlayerObservation, position: str, cards: list[dict], slot: str, depth: int) -> TackleResolution:
    ...
    ranked = sorted(
        ((_identity_score(observation.observed_name, name), name, variants) for name, variants in identities.items()),
        reverse=True,
    )
    ...
    # Card variant is a separate decision. Displayed lineup OVR is only supporting
    # evidence; a one-point boost must never reject an otherwise strong identity.

...
RESEARCH_RATINGS = {
    "SPD", "ACC", "AGI", "COD", "STR", "AWR", "PRC", "BSH", "TAK", "PUR", "POW",
    "MCV", "ZCV", "PRS", "PMV", "FMV", "PBK", "PBP", "PBF", "RBK", "RBP", "RBF",
    "IBL", "LBK",
}
...
def _formula_eligible(card: dict) -> tuple[bool, str]:
    if card.get("extraction_status") != "COMPLETE": return False, "INCOMPLETE_VECTOR"
    ratings = card.get("displayed_ratings") or {}
    if len(ratings) < 15: return False, "INSUFFICIENT_RATINGS"
...
def _same_ovr_cells(cards: list[dict]) -> list[dict]:
    ...
    for (position, archetype, ovr), members in grouped.items():
        ...
        fields = sorted(field for field in RESEARCH_RATINGS if sum(field in (card.get("displayed_ratings") or {}) for card in members) >= 2)
        spreads = {}
        for field in fields:
            values = [card["displayed_ratings"][field] for card in members if field in card["displayed_ratings"]]
            if len(values) >= 2: spreads[field] = max(values) - min(values)
        rows.append({
            "position": position, "archetype": archetype, "ovr": ovr, "cards": len(members),
...
def _focus_extreme_contrasts(cards: list[dict]) -> dict[str, list[dict]]:
    ...
    for (position, archetype, ovr), members in grouped.items():
        ...
        for rating in RESEARCH_RATINGS:
            observed = [(card["displayed_ratings"][rating], card) for card in members if rating in card["displayed_ratings"]]
            if len(observed) < 2: continue
            low_value, low = min(observed, key=lambda item: (item[0], item[1]["external_card_id"]))
            high_value, high = max(observed, key=lambda item: (item[0], item[1]["external_card_id"]))
            if high_value == low_value: continue
            by_position[position].append({
                "position": position, "archetype": archetype, "ovr": ovr, "rating": rating,
                "spread": high_value - low_value,
                "low_card_id": low["external_card_id"], "low_player": low["player_name"], "low_value": low_value,
                "high_card_id": high["external_card_id"], "high_player": high["player_name"], "high_value": high_value,
                "cell_cards": len(members),
            })
    return {
        position: sorted(rows, key=lambda row: (-row["spread"], row["rating"], row["ovr"]))[:50]
...
def build_alpha_readiness(root: Path) -> dict:
    ...
    for card in cards:
        ok, reason = _formula_eligible(card); reasons[reason] += 1
        if ok: eligible.append(card)

    by_position = {}; grouped = defaultdict(list)
    for card in eligible: grouped[card["position"]].append(card)
    cells = _same_ovr_cells(eligible)
    cells_by_position = Counter(row["position"] for row in cells); pairs_by_position = Counter()
    for row in cells: pairs_by_position[row["position"]] += row["pairwise_comparisons"]

    for position, rows in sorted(grouped.items()):
        ovrs = [row["overall"] for row in rows]; archetypes = Counter(row["archetype"] for row in rows)
        pair_count = pairs_by_position[position]
        if len(rows) >= 50 and cells_by_position[position] >= 3: readiness = "READY_NOW"
        elif len(rows) >= 15 and cells_by_position[position] >= 1: readiness = "USABLE_WITH_RESTRICTIONS"
        else: readiness = "SPARSE"
        by_position[position] = {
            "formula_eligible_cards": len(rows), "ovr_range": [min(ovrs), max(ovrs)],
            "archetypes": dict(sorted(archetypes.items())), "same_ovr_archetype_cells": cells_by_position[position],
            "pairwise_natural_experiments": pair_count, "readiness": readiness,
        }
    ...
    return {
        ...
        "formula_eligibility": {"eligible": len(eligible), "excluded": len(cards) - len(eligible), "classification_counts": dict(sorted(reasons.items()))},

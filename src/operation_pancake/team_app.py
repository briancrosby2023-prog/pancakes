...
(()=>{{
 function initTeamDrop(){{
  ...
  if(!form||!input||!zone||zone.dataset.dropReady==='1') return;
  ...
  zone.addEventListener('dragenter',e=>{{if(!isFileDrag(e))return; dragDepth++; zone.style.borderStyle='solid';}});
  zone.addEventListener('dragover',e=>{{if(isFileDrag(e)&&e.dataTransfer)e.dataTransfer.dropEffect='copy';}});
  zone.addEventListener('dragleave',e=>{{if(!isFileDrag(e))return; dragDepth=Math.max(0,dragDepth-1); if(!dragDepth)zone.style.borderStyle='dashed';}});
  zone.addEventListener('drop',e=>{{if(!isFileDrag(e))return; dragDepth=0; zone.style.borderStyle='dashed'; addFiles(e.dataTransfer.files);}});
  zone.addEventListener('click',()=>input.click()); zone.addEventListener('keydown',e=>{{if(e.key==='Enter'||e.key===' '){{e.preventDefault(); input.click();}}}}); add.addEventListener('click',()=>input.click()); input.addEventListener('change',()=>addFiles(input.files));
...
def create_handler(root:Path,**kwargs):
    Base=product_app.create_handler(root,**kwargs); gm=GMProduct(root); roster_path=kwargs.get('roster_path') or root/'.operation_pancake/roster.json'; roster=RosterStore(roster_path,set(gm.cards)); imports=TeamImportStore(root/'.operation_pancake/team_import.json')
    class H(Base):
        def _team_page(self):

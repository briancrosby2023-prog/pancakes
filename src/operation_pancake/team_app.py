...
from operation_pancake.team_import import (
    Candidate,
    IMAGE_TYPES,
    OCRObservation,
    SPECIALIST_SLOTS,
...
(()=>{{
 function initTeamDrop(){{
  const status=document.getElementById('team-drop-status');
  ...
  try{{
   ...
   const required={{form,input,zone,selection,count,list,add,analyze,processing,errors}};
   const missing=Object.entries(required).filter(([,value])=>!value).map(([name])=>name);
   if(missing.length){{setStatus(`DROP HANDLER: ERROR — missing ${{missing.join(', ')}}`);return;}}
   if(zone.dataset.dropReady==='1'){{setStatus('DROP HANDLER: READY');return;}}
   ...
   ['dragenter','dragover','drop'].forEach(type=>window.addEventListener(type,pageGuard,{{capture:true,passive:false}}));
   ['dragenter','dragover','dragleave','drop'].forEach(type=>zone.addEventListener(type,e=>{{if(!isFileDrag(e))return; e.preventDefault(); e.stopPropagation();}},{{passive:false}}));
   zone.addEventListener('dragenter',e=>{{if(!isFileDrag(e))return; dragDepth++; zone.style.borderStyle='solid'; setStatus('DROP HANDLER: FILE DRAG');}});
   zone.addEventListener('dragover',e=>{{if(isFileDrag(e)&&e.dataTransfer)e.dataTransfer.dropEffect='copy';}});
   zone.addEventListener('dragleave',e=>{{if(!isFileDrag(e))return; dragDepth=Math.max(0,dragDepth-1); if(!dragDepth){{zone.style.borderStyle='dashed'; setStatus('DROP HANDLER: READY');}}}});
   zone.addEventListener('drop',e=>{{if(!isFileDrag(e))return; dragDepth=0; zone.style.borderStyle='dashed'; addFiles(e.dataTransfer.files); setStatus('DROP HANDLER: READY');}});
   ...
   setStatus('DROP HANDLER: READY');
  }}catch(error){{
   setStatus(`DROP HANDLER: ERROR — ${{error&&error.message?error.message:'initialization failed'}}`);
  }}
 }}
...
def _ocr(path: Path) -> list[OCRObservation] | None:
    ...
    try:
        ...
        for r in rows:
            ...
            if not text:
                continue
            x, y, w, h = (int(r.get(k) or 0) for k in ("left", "top", "width", "height"))
...
def _extract(state_store: TeamImportStore, gm: GMProduct):
    ...
    for shot in state.screenshots:
        ...
        if obs is None:
            shot["extraction_status"] = "OCR ENGINE UNAVAILABLE"
            continue
        view, found, meta = extract_structured(shot["id"], obs, DEFAULT_REGIONS)
        shot["extraction_status"] = f"OCR READ — {view}"
        shot["view"] = view
        shot["view_confidence"] = meta.get("view_confidence")
        observations[shot["id"]] = meta
        for observed in found:
            c = to_candidate(observed, f"cand-{len(candidates) + 1}")
            candidates.append(match_candidate(c, gm.population))
    state.version = 2
    state.candidates = candidates
    state.team_observations = {"screenshots": observations}
    state_store.save(state)
    return state


def create_handler(root: Path, **kwargs):
    Base = product_app.create_handler(root, **kwargs)
    gm = GMProduct(root)
    ...
    class H(Base):
        ...
        def _team_page(self):
            ...
            if state.candidates:
                body += f'<form method="post" action="/team/confirm"><div class="card"><h2>Review Team</h2><table><tr><th>View</th><th>Slot</th><th>Player</th><th>Observed OVR</th><th>Canonical match</th><th>Status</th></tr>{rows}</table><button>IMPORT TEAM</button></div></form>'
            return product_app.page("Team Setup", body)

        def do_GET(self):
            p = urlparse(self.path).path
            if p == "/setup":
                self.send(self._team_page())
                return
            if p == "/api/team-import":
                self.js({"state": asdict(imports.load())})
                return
            super().do_GET()
        ...
        def do_POST(self):
            ...
            try:
                if p == "/team/upload":
                    ...
                    parts = _multipart(self.headers, body)
                    files = [(fn, ct, data) for name, fn, ct, data in parts if name == "images"]
                    if not files:
                        raise ValueError("Choose at least one image")
                    bad = [fn for fn, ct, data in files if ct not in IMAGE_TYPES or not data]
                    if bad:
                        raise ValueError("Unsupported or empty image file: " + ", ".join(Path(x).name for x in bad) + ". Use PNG, JPEG, WEBP, HEIC, or HEIF.")
                    imports.stage_bytes(files)
                    _extract(imports, gm)
                    self.redir("/setup")
                    return
                if p == "/team/confirm":
                    f = parse_qs(self.rfile.read(int(self.headers.get("Content-Length", "0"))).decode())
                    state = imports.load()
                    byslot = {x.slot: x for x in roster.load()}
                    for c in state.candidates:
                        cid = f.get("card__" + c.id, [""])[0]
                        sl = c.slot.upper().strip()
                        if not cid or not sl:
                            continue
                        card = gm.cards.get(cid)
                        if not card:
                            continue
                        base = "".join(ch for ch in sl if not ch.isdigit())
                        kind = "SPECIALIST" if base in SPECIALIST_SLOTS else "ROSTER"
                ...
                self.send(product_app.page("Team Setup error", f'<div class="card warn">{html.escape(str(e))}</div>'), 400)
                return
            super().do_POST()
...
def main():
    p = argparse.ArgumentParser(prog="operation-pancake-app")
    p.add_argument("--root", type=Path, default=Path.cwd())
    p.add_argument("--host", default="127.0.0.1")
    p.add_argument("--port", type=int, default=8765)
    a = p.parse_args()
    s = ThreadingHTTPServer((a.host, a.port), create_handler(a.root.resolve()))
    print(f"Operation Pancake: http://{a.host}:{a.port}")
    print(f"Team Setup build: {TEAM_SETUP_BUILD}")
    print(f"Team Setup module: {Path(__file__).resolve()}")
    s.serve_forever()

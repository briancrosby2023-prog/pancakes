...
def _safe_name_match(observed, canonical):
    ...
    for seen, expected in zip(a, b, strict=True):
        if seen == expected:
            continue
        if min(len(seen), len(expected)) < 4:
            return False
...
def search_tackle_cards(name, position, cards):
    ...
    if exact:
        ...
        scored = sorted(
            (SequenceMatcher(None, query, normalize_name(player_name)).ratio(), player_name)
        ...
        chosen = [
            ...
            if score >= max(0.78, best - 0.08) and _safe_name_match(name, player_name)
        ]
...
def select_user_tackle_card(candidate, card_id, cards):
    offered = set(
        candidate.match_diagnostics.get("user_name_fallback", {}).get("result_card_ids") or []
    )
...
def _candidate(state, slot):
    found = next(
        (c for c in state.candidates if c.group == "OFFENSE" and c.slot == slot), None
    )
...
def _backup(row: TackleResolution):
    return {
...
def _apply(candidate, rows):
    starter = rows[0]
    candidate.player_name = starter.canonical_player_identity or starter.observed_player_name
    ...
    candidate.provenance = old + [
        ...
        "c3po:pancake-cfb27-name-resolution",
    ]
...
def integrate_offense_tackles(state_store, cards, translator):
    ...
    offense = next(
        (s for s in state.screenshots if s.get("view") == "OFFENSE"), None
    )
    if offense is None:

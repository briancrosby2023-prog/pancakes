...
def _player_choice(player, card_observation=None, *, selected: bool) -> str:
    ...
    return f'<button type="button" class="player-choice{selected_class}" data-slot="{html.escape(player.slot)}" data-program="{html.escape(program)}" data-missing="{1 if missing else 0}" data-art="{html.escape(art)}"><strong class="{name_class}">{html.escape(name)}</strong><span class="choice-ovr">{html.escape(ovr)}</span><span class="choice-program{" program-missing" if missing else ""}">{html.escape(program)}</span></button>'

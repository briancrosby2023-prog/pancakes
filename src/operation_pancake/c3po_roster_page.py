...
def _player_choice(player, card_observation=None, *, selected: bool) -> str:
    ...
    name_class = "choice-name choice-name-long" if len(name) >= 16 else "choice-name"
    return f'<button type="button" class="player-choice{selected_class}" data-slot="{html.escape(player.slot)}" data-program="{html.escape(program)}" data-missing="{1 if missing else 0}" data-art="{html.escape(art)}" data-ovr="{html.escape(ovr)}"><strong class="{name_class}">{html.escape(name)}</strong><span class="choice-ovr">{html.escape(ovr)}</span><span class="choice-program{" program-missing" if missing else ""}">{html.escape(program)}</span></button>'
...
def _position_group(slot: str, view: str | None = None) -> tuple[str, int]:
    ...
    if view == "DEFENSE":
        ...
        if mike_match:
            ...
            return (f"MIKE{depth}", 1) if depth <= 2 else ("MIKE2", depth - 1)
        cb_match = re.fullmatch(r"CB\s*(\d+)", clean_slot)
        if cb_match:
            depth = int(cb_match.group(1))
            return (f"CB{depth}", 1) if depth <= 3 else ("CB3", depth - 2)
    wr_match = re.fullmatch(r"WR\s*(\d+)", clean_slot)
    if wr_match:
        depth = int(wr_match.group(1))
        return (f"WR{depth}", 1) if depth <= 3 else ("WR3", depth - 2)
...
def render_c3po_roster(roster: C3PORoster, programs=None) -> str:
    ...
    for view_index, view in enumerate(VIEWS):
        ...
        for position, rows in _ordered_groups(view, groups):
            ...
            cards.append(
                f'<section class="position-group"><h3>{html.escape(position)}</h3><div class="feature-card">{art_html}<div class="feature-rating"><strong>{html.escape("?" if first_player.displayed_ovr is None else str(first_player.displayed_ovr))}</strong><span>{html.escape(position)}</span></div><span class="feature-slot">{html.escape(first_player.slot)}</span><div class="feature-copy"><span class="feature-program{" program-missing" if first_missing else ""}">{html.escape(first_program_text)}</span></div></div><div class="player-list">{choices}</div></section>'
            )

...
.position-grid{display:grid;grid-template-columns:repeat(6,minmax(0,1fr));gap:26px 12px;align-items:stretch}.position-group{min-width:0}.position-group h3{height:18px;margin:0 0 5px;color:#91a0b0;font-size:11px;font-weight:500;line-height:18px;text-align:center}
...
def _player_choice(player, card_observation=None, *, selected: bool) -> str:
    ...
    return (
        ...
        f'<strong class="{name_class}">{html.escape(name)}</strong><span class="choice-ovr">{html.escape(ovr)}</span>'

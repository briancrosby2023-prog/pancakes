...
def create_handler(root: Path, roster_path: Path | None = None):
    ...
    def enriched():
        rows=[]
        for assignment in store.load(): rows.append((assignment,gm.cards.get(assignment.card_id,{}),gm.rank_by_id.get(assignment.card_id,{})))
        return sorted(rows,key=lambda row:(row[0].position,row[0].slot))

    def replacements(card_id: str, limit: int = 12):
        current=gm.rank_by_id.get(card_id)
        if not current or current.get("score") is None: return []
        pool=[row for row in gm.ranked if row.get("position_family")==current.get("position_family") and row.get("score") is not None and row["score"]>current["score"] and row["card_id"]!=card_id]
        pool.sort(key=lambda row:(-(row["score"]-current["score"]),row.get("position_rank") or 999999))
        return pool[:limit]
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):
            ...
            if parsed.path=="/api/player": self._json(gm.lookup(card_id=q.get("card_id",[None])[0],player_name=q.get("name",[None])[0],position=q.get("position",[None])[0])); return
            if parsed.path=="/api/roster": self._json({"assignments":[asdict(a)|{"card":gm._identity(c),"pancake_score":s.get("score"),"position_rank":s.get("position_rank")} for a,c,s in enriched()]}); return
            if parsed.path=="/api/replacements": self._json({"current":q.get("card_id",[""])[0],"candidates":replacements(q.get("card_id",[""])[0])}); return
            if parsed.path=="/compare":
                left=q.get("current",[""])[0]; right=q.get("candidate",[""])[0]; body='<h1>Compare players</h1><form><input name="current" placeholder="Current card ID" value="%s"><input name="candidate" placeholder="Candidate card ID" value="%s"><button>Compare</button></form>'%(html.escape(left),html.escape(right))
            ...
            if parsed.path=="/replacements":
                current_id=q.get("current",[""])[0]; card=gm.cards.get(current_id); score=gm.rank_by_id.get(current_id,{})
                if not card: self._send(_page("Replacements",'<h1>Unknown roster card</h1>'),404); return
                body='<h1>Replacement discovery</h1><p>Better production-ranked cards in the same position family for <strong>%s</strong>. Price is intentionally not guessed.</p>'%html.escape(card.get("player_name") or current_id)
                candidates=replacements(current_id)
                if not candidates: body+='<div class="card">No higher Pancake-scored replacement is currently available from the production population.</div>'
                else:
                    body+='<table><thead><tr><th>Candidate</th><th>OVR</th><th>Pancake</th><th>Rank</th><th>Score gain</th><th>Action</th></tr></thead><tbody>'
                    for row in candidates:
                        body+='<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>+%.4f</td><td><a href="/compare?%s">Compare</a></td></tr>'%(html.escape(row.get("player_name") or row["card_id"]),row.get("native_overall") or '—',row.get("score"),row.get("position_rank") or '—',row["score"]-score["score"],urlencode({"current":current_id,"candidate":row["card_id"]}))
                    body+='</tbody></table>'
                self._send(_page("Replacements",body)); return
            if parsed.path=="/roster":
                message=q.get("message",[""])[0]; error=q.get("error",[""])[0]; body='<h1>Roster dashboard</h1><p>Owned cards persist locally and stay linked to the canonical CFB27 population.</p>'
            ...
            if name or position:
                needle=name.casefold(); candidates=[c for c in gm.population if (not needle or needle in (c.get("player_name") or "").casefold()) and (not position or c.get("position")==position)]; candidates.sort(key=lambda c:(-(c.get("native_overall") or 0),c.get("player_name") or ""))
                for c in candidates[:100]:
                    s=gm.rank_by_id.get(c["card_id"],{}); add='<form method="post" action="/roster/add"><input type="hidden" name="card_id" value="%s"><input type="hidden" name="position" value="%s"><input name="slot" size="7" placeholder="%s1" required><label><input type="checkbox" name="starter" checked>Starter</label><label><input type="checkbox" name="protected">Protected</label><label><input type="checkbox" name="rerollable">Rerollable</label><button>Add</button></form>'%(html.escape(c["card_id"]),html.escape(c.get("position") or ""),html.escape(c.get("position") or "")); rows.append('<tr><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td><td>%s</td></tr>'%(html.escape(c.get("player_name") or ""),html.escape(c.get("position") or ""),c.get("native_overall") or '—',html.escape(c.get("program") or ""),s.get("score") if s.get("score") is not None else '—',s.get("position_rank") or '—',add))
...
def main() -> None:
    parser=argparse.ArgumentParser(prog="operation-pancake-app"); parser.add_argument("--root",type=Path,default=Path.cwd()); parser.add_argument("--host",default="127.0.0.1"); parser.add_argument("--port",type=int,default=8765); args=parser.parse_args(); server=ThreadingHTTPServer((args.host,args.port),create_handler(args.root.resolve())); print(f"Operation Pancake: http://{args.host}:{args.port}"); server.serve_forever()

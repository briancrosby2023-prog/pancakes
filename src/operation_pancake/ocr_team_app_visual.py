...
from operation_pancake.c3po_team_setup import apply_user_tackle_name, select_user_tackle_card
    ...
    def create_handler(root, **kwargs):
        ...
        class MyTeamHandler(Base):
            ...
            def do_POST(self):
                ...
                if path in {"/team/tackle-search", "/team/confirm"}:
                    ...
                    if path == "/team/tackle-search":
                        ...
                        for candidate in state.candidates:
                            card_id = form.get("card__" + candidate.id, [""])[0].strip()
                            if card_id and candidate.match_diagnostics.get("user_name_fallback"):
                                changed |= select_user_tackle_card(candidate, card_id, gm.population)
                    if changed:
                        imports.save(state)
                        self.redir("/setup")
                        return

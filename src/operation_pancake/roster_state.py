...
class RosterStore:
    ...
    def save(self, rows: list[RosterAssignment]) -> None:
        ...
        payload = {"version": 1, "assignments": [asdict(row) for row in rows]}
        temp = self.path.with_suffix(self.path.suffix + ".tmp")
        temp.write_text(json.dumps(payload, indent=2) + "\n", encoding="utf-8")
        temp.replace(self.path)

    def _validate(self, row: RosterAssignment, rows: list[RosterAssignment], old_slot: str | None = None) -> None:
        if not row.card_id or not row.position or not row.slot:
            raise ValueError("card_id, position and slot are required")
        if self.valid_card_ids is not None and row.card_id not in self.valid_card_ids:
            raise ValueError("unknown canonical card_id")
        for existing in rows:
            if old_slot is not None and existing.slot == old_slot:
                continue
            if existing.slot == row.slot:
                raise ValueError(f"slot {row.slot} is already assigned")
            if existing.card_id == row.card_id:
                raise ValueError("card is already assigned to the roster")

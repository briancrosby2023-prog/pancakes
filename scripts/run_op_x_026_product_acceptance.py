...
def main() -> int:
    ...
    budget.write_text(
        json.dumps(
            [
                ...
                {
                    "card_id": "protected-demo",
                    "net_cost": 1,
                    "score_improvement": 99,
                    "protected": True,
                },
            ],
            indent=2,
    ...
    cases = [
        ...
        run(
            "bad-unknown-player",
            ["operation-pancake-gm", "player", "--card-id", "OPX026-NOT-A-CARD"],
        ),
        run(
            "bad-invalid-price",
            ["operation-pancake-gm", "compare", a, b, "--price", "not-an-int"],
            expected=2,
        ),
        run(
            "role-intelligence",
            ["operation-pancake-gm", "role-board", "TE", "RECEIVING", "--limit", "3"],
        ),
        run(
            "knowledge",
            ["operation-pancake-gm", "knowledge-search", "CFB27 Season 1"],
        ),
    ]
    det_player = run(
        "determinism-player",
        ["operation-pancake-gm", "player", "--card-id", a],
    )
    det_budget = run(
        "determinism-budget",
        ["operation-pancake-gm", "budget", str(budget), "100000"],
    )
    player_stdout = next(x for x in cases if x["name"] == "player")["stdout"]
    budget_stdout = next(x for x in cases if x["name"] == "budget")["stdout"]
    determinism = {
        "player": det_player["stdout"] == player_stdout,
        "budget": det_budget["stdout"] == budget_stdout,
    }
    payload = {
        "milestone": "OP-X-026E",
    ...
    result_path = OUT / "product_acceptance_execution.json"
    result_path.write_text(json.dumps(payload, indent=2, sort_keys=True) + "\n")
    summary = {
        "acceptance_pass": payload["acceptance_pass"],
        "cases": {
            x["name"]: {"exit_code": x["exit_code"], "pass": x["pass"]} for x in cases
        },
        "determinism": determinism,
    }
